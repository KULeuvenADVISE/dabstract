...
class DataAbstract(Abstract):
    ...
    def __iter__(self) -> Generator:
        return parallel_op(self._data, workers=self._workers, buffer_len=self._buffer_len, \
                           return_info=False, **self._kwargs)

    def __call__(self):
        return self.__iter__()

    def get(self, index: Iterable,
            *args: list,
            return_info: bool = False,
            workers: int = 0,
            buffer_len: int = 3,
        ...
        if isinstance(index, numbers.Integral):
            ...
            gen = parallel_op(_data, *args, workers=workers, buffer_len=buffer_len, \
    ...
    def __repr__(self) -> str:
        return class_str(self._data) + "\n data abstract: multi_processing " + str(
            (True if self._workers > 0 else False))
...
class MapAbstract(Abstract):
    ...
    def get(self, index: int,
            return_info: bool = False,
            *arg: List, **kwargs: Dict) -> Union[List, np.ndarray, Any]:
        if isinstance(index, numbers.Integral):
            if index < 0:
...
def Map(data,
        ...
        buffer_len: int = 3,
        *arg: list,
        **kwargs: Dict) -> Union[MapAbstract, DataAbstract, np.ndarray, list]:
...
class SampleReplicateAbstract(Abstract):
    ...
    def get(self, index: int, return_info: bool = False, *arg: List, **kwargs: Dict) -> Union[List, np.ndarray, Any]:
        if isinstance(index, numbers.Integral):
...
def SampleReplicate(data: Iterable,
                    factor: int,
                    lazy: bool = True,
                    workers: int = 1,
                    buffer_len: int = 3,
                    *arg: List,
                    **kwargs: Dict) -> Union[SampleReplicateAbstract, DataAbstract, np.ndarray, list]:
...
class SplitAbstract(Abstract):
    ...
    def _init_split(self):
        ...
        for j in range(len(self._data)):
            num_frames = max(1, int(
                np.floor(((self._sample_len[j] - (self._window_size - 1) - 1) / self._window_size) + 1)))
        ...
        if isinstance(index, numbers.Integral):
            ...
            for k, split_len in enumerate(self._split_len):
                if split_len <= index:
                    ...
                    # get data
                    if self._abstract:
                        data, info = self._data.get(k, return_info=True, *arg, **kwargs, **self._kwargs, read_range=read_range)
                    else:
...
def Split(data: Iterable,
          ...
          workers: bool = 1,
          buffer_len: int = 3,
          *arg: List, **kwargs: Dict) -> Union[SplitAbstract, DataAbstract, np.ndarray, list]:
    ...
    if lazy:
        ...
        return DataAbstract(SplitAbstract(data, split_size=split_size, constraint=constraint,
                                          sample_len=sample_len, sample_period=sample_period, type=type, *arg,
                                          **kwargs),
                            workers=workers,
                            buffer_len=buffer_len)[:]
...
def Select(data,
           ...
           workers: int = 1,
           buffer_len: int = 3,
           *arg, **kwargs) -> Union[SelectAbstract, DataAbstract, np.ndarray, list]:
...
def Filter(data: Iterable,
           filter_fct: Callable,
           lazy: bool = True,
           workers: int = 1,
           buffer_len: int = 3,
           *arg: List, **kwargs: Dict) -> Union[FilterAbstract, DataAbstract, np.ndarray, list]:
    """Factory function to allow for choice between lazy and direct example selection
    """
    _abstract = (True if isinstance(data, Abstract) else False)
    if lazy:
...
class KeyAbstract(Abstract):
    ...
    def get(self, index: int, return_info: bool = False, *arg: List, **kwargs: Dict) -> Union[List, np.ndarray, Any]:
        if isinstance(index, numbers.Integral):
            assert index < len(self)
            try:
                data, info = self._data.get(key=self._key, index=index, return_info=True, *arg, **kwargs,
                                            **self._kwargs)
            except:
                data, info = None, {}
            return ((data, info) if return_info else data)
...
class DictSeqAbstract(Abstract):
    ...
    def concat(self, data: Iterable, intersect: bool = False, adjust_base: bool = True) -> None:
                ...
                for k, key in keys:
                    if self2._lazy[key]:
                        ...
                        if isinstance(data[key],
                                      SeqAbstract):  # if already a SeqAbstract, concat cleaner to avoid overhead
                            for _data in data[key]._data:
                                self2[key].concat(_data)
                        else:  # if not just concat at once
                            self2[key].concat(data[key])
                    else:
                        assert self2[key].__class__ == data[
                            key].__class__, "When using lazy=False, datatypes should be same in case of concatenation."
