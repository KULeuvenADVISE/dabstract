...
class ProcessingChain():
    ...
    def add(self,
            ...
            parameters: Dict = dict()) -> tvProcessingChain:
        """ Add to chain
        """
        # Add new processor
        if isinstance(name, Processor):
            ...
            if name not in ('none', 'None'):
                ...
                if not hasattr(module, name):  # check customs
                    ...
                    assert hasattr(module,
                                   name), 'Processor is not supported in both dabstract.dataprocessor.processors and dabstract.custom.processors. Please check'
            ...
            raise NotImplementedError("Input that you provided does not work for ProcessingChain().")
        return self
                ...
                if hasattr(chain, 'fit'):
                    ...
                    if 'init_subsample' in self._info[k]['parameters']:
                        if self._info[k]['parameters']['init_subsample'] is not None:
                            sel_ind = np.random.choice(np.arange(len(data)), size=int(
                                self._info[k]['parameters']['init_subsample'] * len(data)), replace=False)
                            data = SelectAbstract(data, (lambda x, k: k in sel_ind))
                    data_tmp = DataAbstract(MapAbstract(data, init_processor), workers=workers, buffer_len=buffer_len)
                    # load into memory
                    if load_memory:
