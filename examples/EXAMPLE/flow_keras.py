...
def flow_keras(cg_in=dict(),co_in=dict()):
    ...
    dirs = load_yaml_config(filename=co['dir_conf'],
                            ...
                            walk=True, **cg)
    # -- get_dataset
    data = load_yaml_config(filename=cg['dataset'], dir=os.path.join('configs','db'), walk=True, post_process=dataset_from_config, **dirs)
    ...
    processor = {'data': load_yaml_config(filename=cg['proc_chain_data'], dir=os.path.join('configs', 'dp'), walk=True,post_process=processing_chain),
                 'meta': load_yaml_config(filename=cg['proc_chain_meta'], dir=os.path.join('configs', 'dp'), walk=True,post_process=processing_chain)}
    # --- init keras
    ...
    for fold in range(xval['folds']):
        ...
        if (co['train'] & (not os.path.isfile(savefile_model + "_fold" + str(fold) + "_epochBEST.hdf5"))) | co['overwrite_train']:
            # --- create dir
            ...
            if (not os.path.isfile(savefile_model + '.json')) or co['overwrite_train']:  # remove later version
                model, custom_objects = get_model(model_name=cg['model'],
                                                  ...
                                                  walk=True)
                with open(os.path.join(dirs['results'], "custom_objects.pickle"), 'wb') as f:pickle.dump(custom_objects, f)
                with open(savefile_model + '.json', 'w') as json_file:json_file.write(model.to_json())
            else:
                with open(savefile_model + '.json') as json_file:json_config = json_file.read()
                with open(os.path.join(dirs['results'], "custom_objects.pickle"), "rb") as f:custom_objects = pickle.load(f)  # load
        ...
        if (co['test'] & (not os.path.isfile(savefile_results + ".pickle"))) | co['overwrite_test']:
            ...
            # # --- init processing chain
            savefile_processor = os.path.join(dirs['results'], "processor_fold" + str(fold))  # save directory
            with open(savefile_processor + ".pickle", "rb") as f: processor = pickle.load(f)  # load
            ...
            with open(loadfile_model + '.json') as json_file:json_config = json_file.read()
            with open(os.path.join(dirs['results'], "custom_objects.pickle"), "rb") as f:custom_objects = pickle.load(f)  # load
            model = kr.models.model_from_json(json_config, custom_objects=custom_objects)
